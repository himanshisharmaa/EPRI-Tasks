...
def csv_to_coco(csv_file,image_dir,output_path):
    ...
    for _,row in df.iterrows():
        ...
        for obj in annotation_data["objects"]:
            ...
            if "polygon" in obj:
                ...
                segmentation = [coord for point in obj["line"] for coord in (point["x"], point["y"])]
                x_coords = [point["x"] for point in obj["line"]]
                y_coords = [point["y"] for point in obj["line"]]
                bbox = [min(x_coords), min(y_coords), max(x_coords) - min(x_coords), max(y_coords) - min(y_coords)]
    ...
    output_dir=os.path.join(dir,"Annotations")
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    output_file=os.path.join(output_dir,"CocoAnnotation.json")
